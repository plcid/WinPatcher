...
// (edit imconfig.hpp or '#define IMGUI_USER_CONFIG "myfilename.h" from your build system)
...
struct ImGuiContext;                // Dear ImGui context (opaque structure, unless including imgui_internal.hpp)
...
// - To use something else than an opaque void* pointer: override with e.g. '#define ImTextureID MyTextureType*' in your imconfig.hpp file.
...
// - To use 32-bit indices: override with '#define ImDrawIdx unsigned int' in your imconfig.hpp file.
#ifndef ImDrawIdx
...
#ifdef IMGUI_USE_WCHAR32            // ImWchar [configurable type: override in imconfig.hpp with '#define IMGUI_USE_WCHAR32' to support Unicode planes 1-16]
...
// Add '#define IMGUI_DEFINE_MATH_OPERATORS' in your imconfig.hpp file to benefit from courtesy maths operators for those types.
...
// Flags for InvisibleButton() [extended in imgui_internal.hpp]
...
//-----------------------------------------------------------------------------
// - You generally do NOT need to care or use this ever. But we need to make it available in imgui.hpp because some of our public structures are relying on it.
...
// - This is in order to allow user enabling implicit cast operators between ImVec2/ImVec4 and their own types (using IM_VEC2_CLASS_EXTRA in imconfig.hpp)
...
// ImDrawCallback: Draw callbacks for advanced uses [configurable type: override in imconfig.hpp]
// NB: You most likely do NOT need to use draw callbacks just to create your own widget or customized UI rendering,
// you can poke into the draw list for that! Draw callback may be useful for example to:
//  A) Change your GPU render state,
//  B) render a complex 3D scene inside a UI element without an intermediate texture/render target, etc.
// The expected behavior from your rendering function is 'if (cmd.UserCallback != NULL) { cmd.UserCallback(parent_list, cmd); } else { RenderTriangles() }'
// If you want to override the signature of ImDrawCallback, you can simply use e.g. '#define ImDrawCallback MyDrawCallback' (in imconfig.hpp) + update rendering backend accordingly.
...
// Please keep your copy of dear imgui up to date! Occasionally set '#define IMGUI_DISABLE_OBSOLETE_FUNCTIONS' in imconfig.hpp to stay ahead.
//-----------------------------------------------------------------------------
...
// May be convenient for some users to only explicitly include vanilla imgui.hpp and have extra stuff included.
